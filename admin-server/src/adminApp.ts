...
let sessions: Map<string, SessionData> | null = null;
...
export function createAdminApp(): express.Express {
  ...
  async function logActivity(userId: string, userEmail: string, action: string, details: any = {}) {
    try {
      ...
      // Swallow logging errors in production to avoid user impact
    }
  }
  ...
  function requireAuth(req: any, res: any, next: any) {
    ...
    req.user = session;
    logActivity(session.id, session.email, 'PAGE_ACCESS', {
      page: req.path,
      ipAddress: req.ip,
  ...
  app.get('/', (req: any, res: any) => {
    ...
    const authUrl = `https://login.microsoftonline.com/${process.env.AZURE_TENANT_ID}/oauth2/v2.0/authorize?client_id=${process.env.AZURE_CLIENT_ID}&response_type=code&redirect_uri=${encodeURIComponent(process.env.AZURE_REDIRECT_URI!)}&scope=openid%20profile%20email%20User.Read&response_mode=query`;
    res.send(`
  ...
  app.get('/auth/callback', async (req: any, res: any) => {
    ...
    try {
      ...
      try {
        ...
        // Ignore persistence errors; allow login to proceed if possible
      }
  ...
  app.get('/logout', (req: any, res: any) => {
    ...
    if (sid && sessions?.has(sid)) {
      ...
      logActivity(u.id, u.email, 'LOGOUT', {
        ipAddress: req.ip,
        duration: Date.now() - u.createdAt
      });
      sessions.delete(sid);
    }
    res.redirect('/');
